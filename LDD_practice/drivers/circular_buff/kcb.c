...
#define BUFFER_SIZE 1024 //defining the buffer size for the device
...
static int open_count = 0, i = 0, read_index = 0; // To count the no. of times the device is opened

// Circular buffer for storing log messages (up to 5 messages)
struct log {
    char msg[100];
};
static struct log svar[5];  // Circular buffer with space for 5 messages
...
static ssize_t device_write(struct file *file, const char __user *user_buffer, size_t size, loff_t *offset) {
    ...
    sscanf(device_buffer, "%s", svar[read_index].msg);
    printk(KERN_INFO "simple_device: Received msg=%s\n", svar[read_index].msg);

    // Update the read index in a circular manner (0-4)
...
MODULE_DESCRIPTION("A Simple Circular Buffer Linux Device Driver");
